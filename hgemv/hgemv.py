...
def generate_test_data(trans, M, N, lda, alpha, beta, incx, incy):
    #np.random.seed(42)
    left = 0
    right = 1
    ...
    if trans == 0:
        ...
        vY_real = (vY_real.astype(np.float32) * beta + alpha * np.matmul(mA_real.astype(np.float32), vX_real.astype(np.float32))).astype(np.float16)
        vY[0:M*incy:incy,:] = vY_real
    else:
        vX_real = vX[:,0:M*incx:incx]
        vY_real = vY[:,0:N*incy:incy]
        vY_real = (vY_real.astype(np.float32) * beta + alpha * np.matmul(vX_real.astype(np.float32), mA_real.astype(np.float32))).astype(np.float16)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', action='store', type=float, default=0)
    ...
    generate_test_data(args.trans, args.M, args.N, args.lda, args.alpha, args.beta, args.incx, args.incy)