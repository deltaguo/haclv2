...
def generate_test_data(batch_size, M, N, K, trans_a=0, trans_b=0):
    ...
    for _ in range(batch_size):
        mA = np.random.uniform(-2.0, 2.0, size=(M,K)).astype(np.float16)
        mB = np.random.uniform(-2.0, 2.0, size=(K,N)).astype(np.float16)
        result = np.matmul(mA.astype(np.float32), mB)
...
def main():
    ...
    is_lda_padding = 0
    is_ldb_padding = 0
    is_dot_alpha_add_beta_C =  1
    
    op = AscendOpKernel('../build/ascblasHgemm_ca.o', './ascblasHgemm.json')
